# Project 1: Configurable 0-9 / 9-0 Counter (基礎雙向循環計數器系統)

## 項目簡介 (Project Description)

本項目為 FPGA 數位電路設計之 **Project 1：設計一個可動態切換正向/反向的 4-bit 循環計數器**。

系統核心功能圍繞於單一高效能計數器單元，透過外部控制引腳 `up_down` 進行即時硬體行為調度：當切換為上數時，系統於 `0` 至 `9` 區間做非對稱邊界循環；切換為下數時，則自動逆向由 `9` 倒數至 `0`。此外，本設計導入了非同步重置與防禦性電路設計，確保系統在實際 FPGA 晶片上運作時，具備極高的抗干擾能力與穩健性。
